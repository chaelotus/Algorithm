...
for i in range(T):
    ...
    queue_arr = deque([{'value':item,'index':idx} for idx,item in enumerate(numList)])
    target_idx = queue_arr[M]['index']
    # enumerate : 리스트, 튜플, 문자열, deque 등 반복 가능한 객체를 반복하면서 인덱스와 값을 동시에 가져올 수 있음.
    # 파이썬에서 enumerate쓸때 반드시 (index, value) 순서


    while len(answer_arr) != N:
        # 첫번째 값보다 더 큰 값이 있는지
        # queue_arr 안 요소는 딕셔너리.

        # deque 리스트와 비슷한 시퀀스이기 때문에 max 함수이 key인자를 주면 된다.
        # max(iterable, key=...) 는 key함수의 반환값을 비교해 가장 큰 원소 리턴한다.
        # lambda는 익명함수를 만들 때 사용하는 파이썬 문법. def로 이름 붙인 함수를 정의하지 않고 짧고 간단한 함수를 한줄로 표현할 때 사용.
        # lambda 매개변수들: 표현식
        # deque는 슬라이싱([:])지원 안함.
        if len(queue_arr)>1 and queue_arr[0]['value'] < max(list(queue_arr)[1:],key=lambda x:x["value"])['value']:
            # 배열 맨뒤로
            queue_arr.append(queue_arr.popleft())
        else:
            answer_arr.append(queue_arr.popleft())
    position = next(i for i, item in enumerate(answer_arr) if item['index'] == target_idx)
    # next() : 첫번쨰 일치하는 위치 가져와줌.
    print(position+1)
```

런타임 에러가 났다. (비정상적으로 종료된 경우)  
찾아보니깐 `input()`과 `sys.stdin.readline()`을 혼용해서 사용하면 이런 에러가 발생한다고 한다.
...
- enumerate : `리스트, 튜플, 문자열, deque 등 반복 가능한 객체`를 반복하면서 인덱스와 값을 동시에 가져올 수 있음.
- 파이썬에서 enumerate쓸때 반드시 `(index, value)` 순서

<br/>

> 객체를 가진 deque에서 max 값 찾기
